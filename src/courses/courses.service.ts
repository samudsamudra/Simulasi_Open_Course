        ...
        async createCourse(title: string, description: string, category: string, price: number) {
            if (price < 0) {
        ...
        async updateCourse(id: number, title?: string, description?: string, category?: string, price?: number) {
            if (!id || isNaN(id) || id <= 0) {
            throw new BadRequestException('ID course tidak valid.');
            }
        ...
        async registerStudent(courseId: number, studentId: number) {
            ...
            throw new ForbiddenException('Hanya student yang dapat mendaftar ke course.');
            }
